...
-- Initialize class colors from Blizzard's system -- INTERNAL
local function InitializeClassColors()
    ...
    for classToken, colorData in pairs(RAID_CLASS_COLORS) do
        if colorData.colorStr then
            ...
            CLASS_COLORS[classToken] = string_match(colorData.colorStr, "^[fF][fF](.+)") or colorData.colorStr
        end
    end
end
...
---@class LoolibNoteRendererMixin
local LoolibNoteRendererMixin = {}

--- Initialise renderer state. Idempotent.
...
function LoolibNoteRendererMixin:UpdateRaidRoster()
    ...
    if IsInRaid() then
        for i = 1, GetNumGroupMembers() do
            local name, _, _, _, _, classToken = SecretUtil.SafeGetRaidRosterInfo(i)
            if name then
                -- Store by base name (without server)
                local baseName = string_match(name, "^([^-]+)")
                if baseName then
                    self._raidRoster[string_lower(baseName)] = classToken
                end
                self._raidRoster[string_lower(name)] = classToken
            end
        end
    elseif IsInGroup() then
        ...
        for i = 1, GetNumGroupMembers() - 1 do
            local unit = "party" .. i
            local name = SecretUtil.SafeUnitName(unit)
            local _, classToken = SecretUtil.SafeUnitClass(unit)
            if name and classToken then
                self._raidRoster[string_lower(name)] = classToken
            end
        end
        -- Add player
        local playerName = SecretUtil.SafeUnitName("player")
        local _, playerClass = SecretUtil.SafeUnitClass("player")
        if playerName and playerClass then
            self._raidRoster[string_lower(playerName)] = playerClass
        end
    else
...
---@return string rendered Formatted text with WoW escape sequences
...
---@return string? rendered Rendered text or nil
...
---@return string icon Icon texture string
...
---@return string icon Spell icon texture string
...
---@return string timeStr Formatted time string
function LoolibNoteRendererMixin:RenderTimer(node)
    ...
    if node.minutes and node.minutes > 0 then
        return string_format("%d:%02d", node.minutes, node.seconds or 0)
    else
...
---@return string coloredText Text with colored names
function LoolibNoteRendererMixin:ColorPlayerNames(text)
    ...
    table_sort(names, function(a, b) return #a > #b end)

    for _, name in ipairs(names) do
        local classToken = self._raidRoster[name]
        local color = CLASS_COLORS[classToken]
        if color then
            -- NT-05: Escape pattern-special characters in name for safe gsub
            local escapedName = string_gsub(name, "([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
            -- Case-insensitive pattern matching with word boundaries
            local pattern = "([%s%p]?)(" .. escapedName .. ")([%s%p]?)"
            text = string_gsub(text, pattern, function(pre, match, post)
                -- Check if already colored
                if string_match(pre, "|c") or string_match(post, "|r") then
...
---@return string? hex Hex color without |c prefix
...
---@return string coloredName Colored name
function LoolibNoteRendererMixin:ColorName(name, classToken)
    ...
    if not classToken then
        classToken = self._raidRoster[string_lower(name)]
    end
...
function LoolibNoteRendererMixin:GetRoleIcon(role)
    if type(role) ~= "string" then
        error("LoolibNoteRenderer: GetRoleIcon: 'role' must be a string", 2)
    end
    return ROLE_ICONS[string_upper(role)] or ""
end
...
---@param r number|string Red (0-1) or hex string
---@param g number? Green (0-1)
---@param b number? Blue (0-1)
---@return string coloredText Colored text with escape codes
function LoolibNoteRendererMixin:ColorText(text, r, g, b)
    ...
    if type(r) == "string" then
        ...
        hex = string_gsub(hex, "^|c[fF][fF]", "")
        hex = string_gsub(hex, "^[fF][fF]", "")
    else
        -- Convert RGB to hex
        hex = string_format("%02X%02X%02X",
            math_floor((r or 1) * 255),
            math_floor((g or r or 1) * 255),
            math_floor((b or r or 1) * 255))
    end
...
---@param text string Input text
---@return string stripped Text without color codes
function LoolibNoteRendererMixin:StripColors(text)
    if not text then
        return ""
    end
    text = string_gsub(text, "|c[fF][fF]%x%x%x%x%x%x", "")
...
---@param text string Input text
---@return string stripped Text without textures
function LoolibNoteRendererMixin:StripTextures(text)
    if not text then
        return ""
    end
    return string_gsub(text, "|T.-|t", "")
end

--- Strip all formatting (colors and textures)
---@param text string Input text
---@return string stripped Plain text
...
---@return table renderer Renderer instance
...
---@return table renderer Default renderer singleton
