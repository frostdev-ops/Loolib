...
assert(LoolibMixin, "Loolib/Core/Mixin.lua must be loaded before CanvasSync")
assert(LoolibCallbackRegistryMixin, "Loolib/Events/CallbackRegistry.lua must be loaded before CanvasSync")
...
function LoolibCanvasSyncMixin:_ComputeDelta(oldState, newState)
    ...
    for _, elementType in ipairs(elementTypes) do
        ...
        if type(oldElements) == "table" then
            for i, elem in ipairs(oldElements) do
                if elem.syncId then
        ...
        if type(newElements) == "table" then
            for i, elem in ipairs(newElements) do
                if elem.syncId then
...
function LoolibCanvasSyncMixin:_RegisterMessages()
    ...
    AddonMessage.Comm:RegisterComm(self._syncPrefix, function(prefix, msg, distribution, sender)
