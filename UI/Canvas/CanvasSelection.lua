...
local _LOOLIB_VERSION = 1 ---@diagnostic disable-line: unused-local
...
---@class LoolibCanvasSelectionMixin : LoolibCallbackRegistryMixin
...
function LoolibCanvasSelectionMixin:FindElementAt(x, y)
    ...
    if self._shapeManager then
        -- no-op: Shape hit testing would need to be implemented in CanvasShape
...
local function LoolibCreateCanvasSelection()
    local selection = {}
    Loolib.Mixin(selection, LoolibCanvasSelectionMixin)
    selection:OnLoad()
    return selection
end
