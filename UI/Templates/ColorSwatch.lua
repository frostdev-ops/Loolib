...
-- Read alpha from the Blizzard color picker.
-- LuaLS can incorrectly surface `OpacitySliderFrame` as an undefined global
-- through the `ColorPickerFrame:GetColorAlpha()` API stub, so isolate it here.
local function GetColorPickerAlpha()
    if ColorPickerFrame and ColorPickerFrame.GetColorAlpha then
        ---@diagnostic disable-next-line: undefined-global
        return ColorPickerFrame:GetColorAlpha()
    end

    local opacitySliderFrame = _G and _G.OpacitySliderFrame
