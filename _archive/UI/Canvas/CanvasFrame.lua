...
---@class LoolibCanvasFrameMixin : Frame
---@field _brushManager any
---@field _shapeManager any  -- CanvasShape manager
---@field _textManager any  -- CanvasText manager
---@field _iconManager any  -- CanvasIcon manager
---@field _imageManager any
...
function LoolibCanvasFrameMixin:_CreatePools()
    ...
    self._dotPool = CreateFramePool("Frame", self._content, nil, function(_, frame)
        frame:Hide()
    ...
    self._shapePool = CreateFramePool("Frame", self._content, nil, function(_, frame)
        frame:Hide()
        frame:ClearAllPoints()
        if frame.texture then
            frame.texture:Hide()
    ...
    self._textPool = CreateFramePool("Frame", self._content, nil, function(_, frame)
        frame:Hide()
        frame:ClearAllPoints()
    ...
    -- Icon pool (for icons)
    self._iconPool = CreateFramePool("Frame", self._content, nil, function(_, frame)
        frame:Hide()
        frame:ClearAllPoints()
        if frame.texture then
            frame.texture:Hide()
    ...
    self._imagePool = CreateFramePool("Frame", self._content, nil, function(_, frame)
        frame:Hide()
        frame:ClearAllPoints()
        if frame.texture then
            frame.texture:Hide()
...
function LoolibCanvasFrameMixin:_OnUpdate(_)
...
function LoolibCanvasFrameMixin:_ShowTextInput(x, y)
    ...
    GlobalBridge:RegisterStaticPopup("Loolib", "LOOLIB_CANVAS_TEXT_INPUT", {
        text = "Enter text:",
        button1 = "OK",
        button2 = "Cancel",
        hasEditBox = true,
...
local function LoolibCreateCanvasFrame(parent)
    local canvas = {}
    Loolib.Mixin(canvas, LoolibCanvasFrameMixin)
    canvas:OnLoad()
